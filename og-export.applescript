on run argv
	...
	set theBaseName to do shell script "basename -s .graffle \"" & (item 1 of argv) & "\""
	...
	tell application "OmniGraffle 5"
		...
		on error
			try
				save document (theBaseName & ".graffle") as (item 3 of argv) in file theOutputFile
				close document (theBaseName & ".graffle")
			on error
				activate
