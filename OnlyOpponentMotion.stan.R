...
relevant <- splits %v% c("n_cw", "n_obs", "normalized_energy", "norm_energy_diff")

# the constant 0..... determined by regression on norm_diff
# me <- chain("motion_energy.csv", read.csv, add_energies)
# lm( norm_diff ~ I(content*target_number_all) - 1, data=me)$coef
# lm( energy_diff ~ I(content*target_number_all) - 1, data=me)$coef
motion_energy_scale <- 0.05136
energy_diff_scale <- 0.01787975
lapse_limit <- 0.05
