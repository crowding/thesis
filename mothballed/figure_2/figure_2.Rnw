...
library(plyr)
load("../pools/occlusion.calcs.RData")
@
...
<<results=hide, echo=false,cache=true>>=
## let's look at "number of targets at threshold"
with(new.env(), {
  load("../pools/All_constant.calcs.Rdata")

  n.targets.sim <- adply(individual.fit2$sim@coef, 1, function(coefs) {
    output <-
    find.intercept(
                   individual.fit2$model,
                   cases=conditions,
                   'log.target.spacing',
                   coef=coefs,
                   )
    output <- transform(output, n.targets.threshold=2*pi*trial.motion.process.radius / exp(log.target.spacing))
  output
  })

  n.targets.summary <- ddply(n.targets.sim, condition.columns,
                             with, c(mean = mean(n.targets.threshold),
                                     sd = sd(n.targets.threshold)))
  n.targets.summary <<- with(n.targets.summary, n.targets.summary[order(subject, trial.motion.process.radius),])
})
#upshot, there is no clean effect either way. 
#DT decreases target number at larger eccentricities
#while PM increases (both have slope pretty close to 1, though, really.) 
@
...
<<results=hide, echo=false, cache=true>>=
library(plyr)
example.trial <- trials[which(occluded.trials)[1],]
arcAngle <- join(example.trial, session.occluders)$arcAngle
contrast <- join(example.trial, session.occluders)$contrast
#The occluder was darker than the background with contrast of $\Sexpr{contrast*100}\%$.
@

Trial structure and timing were as in \nameref{sec:constant}, but with the addition of an occluder that was present or absent on either side of the screen. The occluder was a `C' shape with an opening subtending $\Sexpr{360 - arcAngle * 180/pi}^\circ$ on either the left or right of the screen and covering all eccentricities that a target might appear at. The stimuli are shown schematically in the legend of \autoref{fig:occlusionPSE}, but for the data reported here, the occluder was not visible (i.e. it had the same luminance as the background.)
...
If the perceived motion direction were driven by element number rather than element spacing, we might first expect the measured critical spacing to decrease when most targets are occluded. For Subject DT the smallest spacing at PSE were observed in the session where the unoccluded window was in the left hemifeld. In the left hemifield spacing at PSE did not reliably differ between occluded and unoccluded conditions (parametric bootstrap, \Sexpr{with(subset(difference.over.eccentricity, subject=="dt" & visibilityCondition=="left"), pretty.pval(difference.positive, 1/difference.n))}) In the right hemifield, the spacing at PSE increased for the occluded conditions (\Sexpr{with(subset(difference.over.eccentricity, subject=="dt" & visibilityCondition=="right"), pretty.pval(1-difference.positive, 1/difference.n))}). [Author PBM did not show a significant difference in critical spacing between occluded and unoccluded stimuli \todo{this is apparently the case for most data but I need to re-run using the final set of stimulus parameters}]

<<results=hide, echo=false, cache=true>>=
gb.reduction.left.p <- with(subset(difference.over.eccentricity,
                                   subject=="gb" & visibilityCondition=="left"),
                            pretty.pval(difference.positive, 1/difference.n))
gb.reduction.right.p <- with(subset(difference.over.eccentricity,
                                   subject=="gb" & visibilityCondition=="left"),
                            pretty.pval(difference.positive, 1/difference.n))
gb.reduction.mean <- with(subset(differences, subject=="gb"), mean(difference))
gb.reduction.sd <- with(subset(differences, subject=="gb"), sd(difference))
gb.reduction.mean.sd <- sd(with(subset(differences, subject=="gb"),
                                 daply(differences, "X1", with, mean(difference))))
@

On the other hand, GB showed a significant decrease in critical spacing for occluded stimuli shown on both sides of the screen (\Sexpr{gb.reduction.left.p}, left; \Sexpr{gb.reduction.right.p}, right.) Over all eccentricities and both hemifields, the average PSE for occluded conditions was $\Sexpr{percentage(exp(gb.reduction.mean))} \pm \Sexpr{percentage((exp(gb.reduction.sd) - 1) * exp(gb.reduction.mean))}$ that of unoccluded conditions. \todo{this standard error includes both the estimation uncertainty and the scatter between different conditions, all in one pool. Is that confusing?} This reduction is considerable, but nonetheless there are more elements visible in one hemifield during unoccluded stimuli at PSE than there are in the occluded condition at PSE. In other words the reduction in PSE is not great enough to be accounted for solely by target number.
...
<<results=hide, echo=false, cache=true>>=
percentage <- function(p,digits=2, nsmall=0, ...) paste("\\\\ensuremath{",format(p*100, digits=digits, nsmall=nsmall, ...),"\\\\%}")
with(trials[considered.trials,], {
	p.appeared <<- mean(appearances != 0)
	p.disappeared <<- mean(disappearances != 0)
	p.both <<- mean(appearances != 0 & disappearances != 0)
	p.neither <<- mean(appearances == 0 & disappearances == 0)
})
@

Of the trials with an occluder reported on in \autoref{fig:occlusionPSE},  \Sexpr{percentage(p.appeared)} of trials contained an element appearance, \Sexpr{percentage(p.disappeared)} of trials contained an element disappearance, \Sexpr{percentage(p.both)} of trials contained both, and \Sexpr{percentage(p.neither)} contained neither. No trials contained more than one appearance or disappearance event.

\todo{This is where the model exploration used to start, before the business with the ellipses. Move the data in this para up or eliminate it?} We first added the predictor $n_{max}$ which was the the maximum number of targets visible during the stimulus presentation. $n_{max}$ was a significant predictor of subjects' responses (Likelihood ratio test, \Sexpr{pretty.pval(pval.nmax)}). Furthermore, $n_{max}$ was a significant predector among each subset of trials delineated by subject and occlusion condition; (\Sexpr{blocked.pvals.max.visible.readout}). However adding interaction terms for subject and hemifield did not improve the fit, so we did not block $n_{max}$ by those factors.

<<results=hide, echo=false, cache=true>>=
pval.nmax <- anova(intercepts.model, max.visible.model, test="Chisq")$P[[2]]
blocked.pvals.max.visible <-
    ddply(cbind(trials[occluded.trials,], ix=1:sum(occluded.trials)),
    c('subject', 'visibilityCondition'),
    function(block) c(p=chisq.subset(intercepts.model, max.visible.model,
                                 subset=block$ix)[[2,"p"]])
    )
blocked.pvals.max.visible.readout <- 
  do.call(paste,
          c(alply(blocked.pvals.max.visible, 1, with,
                  paste(toupper(subject), " ", visibilityCondition, ", ", pretty.pval(p), sep="")),
            sep="; "))
@
...
library(gmodels)
library(arm)
appearances.hyp <- estimable(appearances.model, c(appearances=1), 0)
appearances.subject.pval <- anova(appearances.model, appearances.subject.model, test="Chisq")$P[[2]]
...
with(data.frame(appearances.subject.max.sim@coef), {
  dt.ratio <<- mean(appearances/max.visible)
  dt.ratio.sd <<- sd(appearances/max.visible)
  gb.ratio <<- mean((appearances + subjectgb.appearances)
                    /(max.visible + subjectgb.max.visible))
  gb.ratio.sd <<- sd((appearances + subjectgb.appearances)
                     /(max.visible + subjectgb.max.visible))
})
@

We then considered the effect of targets appearing from behind the occluder, adding the regressor $n_{app}$, the number of appearance events during each trial. The coefficient for $n_{app}$ was negative ($\beta_{n_{app}} = \Sexpr{latex.format(appearances.hyp$Est, digits=2)} \pm \Sexpr{latex.format(appearances.hyp$Std, digits=2)}$), indicating that appearances increased the rate at which the subjects answered incorrectly (i.e. in agreement with the local direction of motion.) Furthermore, there was a significant interaction \todo{is this the right phrasing? adding interaction coeficients between subject and appearances significantly improved model fit.} between number of appearances and subject identity (\Sexpr{pretty.pval(appearances.subject.pval)}), indicating that appearance events affected different subjects' responses differently. The effect of appearances was considerable compared to the effect of adding targets to the display; for subject GB, the coefficient associated with $n_{app}$ was $\Sexpr{latex.format(gb.ratio,digits=2)} \pm \Sexpr{latex.format(gb.ratio.sd,digits=2)}$ times the coefficient of $n_{max}$; for subject DT the ratio was $\Sexpr{latex.format(dt.ratio,digits=2)} \pm \Sexpr{latex.format(dt.ratio.sd,digits=2)}$. In other words shifting the initial target position so that an appearance occurs midway through the trial had more effect than adding two or three targets to the unoccluded portion of the display. Differences in the effect of appearances may be due to differing strategies employed by the subjects, and may help to explain the differing effects of occlusion on the PSE values measured for \autoref{fig:occlusionPSE}.

<<results=hide, echo=false, cache=true>>=
dt.disappearances.hyp <- estimable(disappearances.subject.model, c(disappearances=1),0)
gb.disappearances.hyp <- estimable(disappearances.subject.model, c(disappearances=1,`subjectgb:disappearances`=1),0)
disappearances.pval <- anova(disappearances.model, appearances.subject.model, test="Chisq")$P[[2]]
disappearances.subject.pval <- anova(disappearances.subject.model, disappearances.model, test="Chisq")$P[[2]]
@

As for disappearances, these also had an effect, although less strongly than appearances. Interestingly, the pattern of effects for disappearances was nearly the inverse of that for appearances: a disappearance generally increased the likelihood of subjects' answering correctly. \todo{I could reference something from MOT literature, like Scholl and Pylyshyn, in assonance with this.} Moreover, the improvement was significant for subject GB ($B_{dis,GB} = \Sexpr{latex.format(gb.disappearances.hyp$Est, digits=2)} \pm \Sexpr{latex.format(gb.disappearances.hyp$Std, digits=2)}$, \Sexpr{pretty.pval(gb.disappearances.hyp$P)}) but did not reach significance for subject DT ($B_{dis,GB} = \Sexpr{latex.format(dt.disappearances.hyp$Est, digits=2)} \pm \Sexpr{latex.format(dt.disappearances.hyp$Std, digits=2)}$, p = \Sexpr{pretty.pval(dt.disappearances.hyp$P)}) Overall, including disappearances significantly improved the model fit (\Sexpr{pretty.pval(disappearances.pval)}) and the fit was not further improved by stratifying by  subject (\Sexpr{pretty.pval(disappearances.subject.pval)})\todo{There's a "significant" interaction with GB, left/right side and disappearances; but it's silly to get that specific esp. to get that specific for one subject and not another}.
